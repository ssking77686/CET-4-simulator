...
只是一个 HTML 文件，打开浏览器就能用，无需联网，完全免费。不用装任何东西。不用配置 API Key。**隐私安全**：PDF 不上传服务器，全部在浏览器本地处理。
...
* **PDF 识别不是 100%**：依赖 PDF.js，遇到双栏排版、扫描件、图片型 PDF 会提取失败。市面上大多数电子版真题是单栏的，问题不大，但预览确认这步一定不能省。后续我会思考解决方法。
