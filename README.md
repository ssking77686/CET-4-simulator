...
界面全面翻新。之前那版说实话有点丑，自己用着都觉得憋屈。花了一天时间把整套 UI 重写了一遍。

**视觉重设计**

参考 Apple Human Interface 从头做了视觉系统。CSS 变量驱动，毛玻璃顶栏（`backdrop-filter` 模糊 + 半透明底），所有颜色收敛到一套暖灰 + 主题蓝的调色盘里。之前一堆 `body.dark .xxx` 的覆写规则全删了，现在深色模式只靠修改变量值，干净了很多。

**控件细节**

- 按钮全改成胶囊形（`border-radius: 999px`），hover 的时候有个微弹跳
- 选择题的选项前面加了圆形字母徽章（A/B/C/D），选中的会有蓝色填充
- 计时器加了顶部进度条，时间紧张的时侯会变红闪烁
- 选词填空和段落匹配的词库按钮也统一了 pill 风格，已用的不可再选会灰掉
- 成绩页的评分卡换成了暗色底 + 大字母等级 + 统计行式布局，比之前的四列网格好看多了

**深色模式**

之前深色模式写了快 100 行覆写样式，每次改组件都要同步两套。现在所有颜色抽到 `:root` 里，`body.dark` 只换 20 个变量值就全切了。顶栏的 ◐ 按钮一键切换，存到 localStorage 记住偏好。

**暗号**

如果你在看源码——大部分改动在 `<style>` 块里，JS 里 `renderMatchQ`、`renderAllListening`、`renderCarefulQ`、`renderClozeQ` 这几个函数的模板从内联 `style=` 换成了 CSS class。功能逻辑没动。
